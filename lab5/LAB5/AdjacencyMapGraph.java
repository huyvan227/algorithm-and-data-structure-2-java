...
public class AdjacencyMapGraph<V, E> implements Graph<V, E> {
	...
	public static void main(String[] args) throws IOException{
		AdjacencyMapGraph matrix = new AdjacencyMapGraph(false);
		...
		for (int i = 0; i< N;i++){
			for (int j = 0; j<N;j++){
				int num;
				num = Integer.parseInt(adjmatrix[i][j]);
				if (num == 0 || num == 99999){
					continue;
				}
				else{
					matrix.insertEdge(matrix.insertVertex(i),matrix.insertVertex(j),adjmatrix[i][j]);
				}
