...
public class LinkedPositionalList<E> implements PositionalList<E> {
	// ---------------- nested Node class ----------------
	/**
	 * Node of a doubly linked list, which stores a reference to its element
	...
	private static class Node<E> implements Position<E> {
		 ...
		 * @param p
		 *                reference to a node that should precede the
		 *                new node
		 * @param n
		 *                reference to a node that should follow the new
		 *                node
		 */
	...
	public LinkedPositionalList() {
		...
		trailer = new Node<>(null, header, null); // trailer is preceded
								// by header
	...
	/**
	 * Returns the given node as a Position, unless it is a sentinel, in
	 * which case null is returned (so as not to expose the sentinels to the
	 * user).
	 */
	 ...
	 * predecessor and successor should be neighboring each other prior to
	 * the call.
	 *
	 * @param pred
	 ...
	 *                inserted
	 * @param succ
	 *                node just after the location where the new element is
	 *                inserted
	 * @return the new element's node
	 */
	private Position<E> addBetween(E e, Node<E> pred, Node<E> succ) {
		Node<E> newest = new Node<>(e, pred, succ); // create and link a
								// new node
		pred.setNext(newest);
		succ.setPrev(newest);
		size++;
	...
	public Position<E> addLast(E e) {
		return addBetween(e, trailer.getPrev(), trailer); // just before
									// the
									// trailer
	}
	 ...
	 * implicit reference to the containing list, allowing us to call the
	 * list's methods directly.
	 */
	private class PositionIterator implements Iterator<Position<E>> {

		/**
		 * A Position of the containing list, initialized to the first
		 * position.
		 */
		 ...
		 * @throws IllegalStateException
		 *                 if remove was already called since recent
		 *                 next
		 */
		public void remove() throws IllegalStateException {
			...
			LinkedPositionalList.this.remove(recent); // remove from
									// outer
									// list
			recent = null; // do not allow remove again until next
					// is called
		}
	...
	public Iterable<Position<E>> positions() {
		return new PositionIterable(); // create a new instance of the
						// inner class
	}
	...
	// Debugging code
	/**
	 * Produces a string representation of the contents of the list. This
	...
	public static void main(String[] args) throws IOException{
		...
		for (int i = 0; i< N;i++){
			...
			for (int j = 0; j<=N;j++){
				...
				else if ( Integer.parseInt(adjmatrix[i][j]) == 99999){
					continue;
				}
				else if ( Integer.parseInt(adjmatrix[i][j]) == 0){
					continue;
				}
				else {
					System.out.print(j+ " ");
