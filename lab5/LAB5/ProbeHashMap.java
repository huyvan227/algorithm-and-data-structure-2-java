...
public class ProbeHashMap<K, V> extends AbstractHashMap<K, V> {
	private MapEntry<K, V>[] table; // a fixed array of entries (all
					// initially
					// null)
	 ...
	 * returning -(a+1) where a is the index of the first empty or available
	 * slot that can be used to store a new such entry.
	 *
	 * @param h
	...
	private int findSlot(int h, K k) {
		...
		do {
			if (isAvailable(j)) { // may be either empty or defunct
				if (avail == -1)
					avail = j; // this is the first
							// available slot!
				if (table[j] == null)
					break; // if empty, search fails
						// immediately
	...
	/**
	 * Returns value associated with key k in bucket with hash value h. If
	 * no such entry exists, returns null.
	 * 
	 * @param h
